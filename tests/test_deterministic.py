...
deterministic_fnames.append(
    "stochastic_piston"
)  # `stochastic_piston` is basically deterministic
...
class TestFunctionOutputs:
    ...
    def test_outputs_match(self, fname, scale01):
        ...
        assert np.allclose(
            y_r, y_py, rtol=1e-9, atol=1e-10
        ), f"{fname} (scale01={scale01})"
...
class TestEdgeCases:
    ...
    def test_boundary_values_equal(self, fname):
        ...
        assert np.allclose(
            y_lower_r, y_lower_py, equal_nan=True
        ...
        assert np.allclose(
            y_upper_r, y_upper_py, equal_nan=True
