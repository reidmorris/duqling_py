...
def run_one_sim_case(
    ...
    pred_funcs = pred_funcs + [None] * (
        len(fit_funcs) - len(pred_funcs)
    )  # Pad with None
    ...
    for ii, (fit_func_i, pred_func_i) in enumerate(zip(fit_funcs, pred_funcs)):
        method_name = (
            method_names[ii]
            if method_names and ii < len(method_names)
            else f"method{ii}"
        )
        ...
        if not isinstance(preds, (list, np.ndarray)):
            raise ValueError(
                "fit/pred functions must return a vector, matrix, or a list."
            )
        ...
        if isinstance(preds, np.ndarray):
            ...
            for conf in conf_level:
                ...
                bounds = np.array(
                    [
                        np.quantile(preds[:, i], [alpha / 2, 1 - alpha / 2])
        ...
        # If preds is a list with a special structure
            ...
            if "samples" not in preds:
                raise ValueError(
                    "If pred/fit function returns a list, the samples field must be specified."
                )
