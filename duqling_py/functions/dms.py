...
def dms_complicated(x, scale01=True):
    ...
    Statistics and Computing, 8.4 (1998): 337-346.
    """
    return 1.9 * (1.35 + np.exp(x[0]) * np.sin(13 * (x[0] - 0.6)**2) * 
                  np.exp(-x[1]) * np.sin(7 * x[1]))

# Register functions with metadata
for func_name in ["dms_simple", "dms_radial", "dms_harmonic", "dms_additive", "dms_complicated"]:
    register_function(
        fname=func_name,
        input_dim=2,
        input_cat=False,
